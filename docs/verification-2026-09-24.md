...
- **SOEST の火山のページ**: `/GG/HCV/` から `/earthsciences_archive/HCV/` へ移転していたが、2026-09-21・24 とも www.soest.hawaii.edu のサーバー自体が応答しない。週1回の検査が毎週失敗通知を出さないよう、ʻĀina 01・04 の2本は Wayback Machine の2026年8月の保存へ差し替え、出典の見出しにそう書いた。サーバーが戻ったら元の URL に戻す
...
| Kū ka Pūnohu ʻUla | 0 | 4 | 5 | 名の前の呼びかけは Ē（PE ē 1.）、kumu の答えの歌の引用は a luna、別の詠唱の注をこの歌の pūnohu に当てはめない（PE pūnohu 2. は地面近くの虹） |
...
## 最終レビュー（fable-advisor）

今夜の11コミットをまとめて読ませた。最大のリスクとして、SOEST が落ちている間は週1回のリンク検査が毎週失敗し、英文の失敗メールが持ち主に届くと指摘された。上の差し替えで対処した。ほかに、DDR 011 追記の「360px 以下では2段」が CSS の規則のように読める点（実測の結果として書き直した）、文字サイズの変更でスクロールがどれだけ増えたかの実測が無い点（曲ページ +1.8%、ʻŌlelo 07 +3.0% を DDR 011 に追記）、この表に Kū の行が無かった点を直した。確認問題の並べ替え（DDR 014）と、Emerson の4曲が歌詞の規約を満たすことは妥当とされた。

## 検証の結果（最終）

- `npm test`: 29 files / 507 tests すべて pass
- `npm run lint`（tsc）: エラーなし
- `npm run build`: 成功（404.html のコピーまで）
